...
export default function DontKnowMyModel(props)  {
    let form = createRef();
    ...
    function submitForm(e){
        ...
        if(fd.get('name')?.length && fd.get('phone').split(/[-_()^\s*$]+/).join('').length === 12){
            ...
            fd.append('type', 'Я не знаю модель устройства');
            if(window?.ym){
    ...
    return(
        <div className={`ask-order ${props.isModalOpen ? 'd-flex' : false}`}>
            <div className='ask-overlay' onClick={()=>{props.toggleModal(false)}}/>
            <div className="container ask-block">
                <div className="ask-block-close" onClick={()=>{props.toggleModal(false)}} />
                {
                    isOk === 1
                    ...
                    : isOk === 2
                        ? <form className="d-flex flex-column" ref={form} onSubmit={(e)=>{submitForm(e)}}>
                            <div className="d-flex flex-column">
                                <p className="request">Я не знаю<br/>модель устройства</p>
                                <span className="request2">Оставьте свои контактные данные и мы выясним это вместе</span>
                            </div>
